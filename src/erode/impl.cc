...
std::vector<cv::Mat> erode(const cv::Mat& src_erode, const cv::Mat& src_dilate) {
     ...
     */
    cv::Mat dst_erode, dst_dilate;//输出

    // TODO: 在这里实现你的代码
    cv::Mat gray1, gray2, binary1, binary2;//存储
    //转换为灰度图像
    ...
    //函数生成一个 3x3 的矩形核，用于腐蚀和膨胀操作。
    cv::Mat element = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3));
    //腐蚀二值图像
    cv::erode(binary1, dst_erode, element, cv::Point(-1, -1), 3);
    //膨胀二值图像
    cv::dilate(binary2, dst_dilate, element, cv::Point(-1, -1), 3);
